...
// Keep REPORT.md's generated block in sync (between the SCOREBOARD markers), when scoring both modes.
const reportFile = path.join(ROOT, 'REPORT.md');
if (modes.length === 2 && fs.existsSync(reportFile)) {
  const rep = fs.readFileSync(reportFile, 'utf8');
  const B = '<!-- SCOREBOARD:BEGIN (generated by harness/score.mjs) -->', E = '<!-- SCOREBOARD:END -->';
  const i = rep.indexOf(B), j = rep.indexOf(E);
  if (i >= 0 && j > i) fs.writeFileSync(reportFile, `${rep.slice(0, i + B.length)}\n${md.replace(/^# Scoreboard\n/, '').replace(/^## /gm, '### ')}\n${rep.slice(j)}`);
}
console.log(md);
